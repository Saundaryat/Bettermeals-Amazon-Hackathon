...
const PhaseIcon = ({ phase, isActive }: { phase: string; isActive: boolean }) => {
    // Responsive icon size handles by className or separate logic, here using simple check or class
    // Simpler to control via parent or use currentColor and size prop. 
    // Let's use flexible size if lucide supports it (it accepts number or string).
    // Or better, set size to a larger default and control via CSS? 
    // Lucide size prop sets width/height attributes.
    return (
        <span className="md:scale-150 block transform origin-center">
            {/* Original switch logic but wrapped for scaling */}
            <IconContent phase={phase} isActive={isActive} />
        </span>
    )
};

const IconContent = ({ phase, isActive }: { phase: string; isActive: boolean }) => {
    const iconSize = 20; // Base size, scaled up by parent wrapper on desktop
...
interface CyclePhaseVisualizerProps {
    activePhase: string | null;
    onPhaseChange: (phase: string | null) => void;
}

export default function CyclePhaseVisualizer({ activePhase, onPhaseChange }: CyclePhaseVisualizerProps) {
    // const [activePhase, setActivePhase] = useState<string | null>(null); // Removed internal state
    ...
    return (
        <div className="flex flex-col items-center justify-center relative w-full max-w-[220px] md:max-w-[350px] lg:max-w-[400px] mx-auto aspect-square">
                ...
                {phases.map((phase, index) => {
                    ...
                    return (
                        <g key={phase.name}
                            ...
                            onClick={() => onPhaseChange(phase.name === activePhase ? null : phase.name)}
                        >
                            <path
                                d={d}
                                fill="none"
            ...
            {/* Lady Image in Center - Scaled up to overlap */}
            <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[80%] h-[100%] pointer-events-none z-50 flex items-center justify-center">
                <img src={cycleLady} alt="Cycle Syncing Lady" className="w-full h-full object-contain drop-shadow-xl scale-110 md:scale-125 origin-center" />
            </div>
            ...
            <div className="absolute inset-0 pointer-events-none">
                {phases.map((phase) => {
                    ...
                    return (
                        ...
                        >
                            <div className={`rounded-full p-1.5 md:p-3 mb-0.5 md:mb-2 ${isActive ? 'bg-white/20 backdrop-blur-sm' : ''}`}>
                                <PhaseIcon phase={phase.name} isActive={isActive} />
                            </div>
                            {/* Label */}
                            <div className={`text-[9px] md:text-lg text-center font-medium ${isActive ? 'opacity-100' : 'opacity-70'} transition-opacity leading-tight`}>
                                <span className="block text-gray-800 tracking-tight md:tracking-normal">{phase.label}</span>
                                <span className="block text-[7px] md:text-sm text-gray-500">{phase.days}</span>
                            </div>
                        </div>
                    )
                })}
